...
from app.image_research.core.search_planner import SearchPlanner, compact_search_query
...
class ImageResearcher:
        ...
        try:
            ...
            if not candidates:
                if (
                    ...
                    warnings.append(
                        "Entity candidates failed license/relevance filtering; falling back to stock provider."
                    )
                    stock_candidates = await self._search_stock(search_plan, request, warnings)
                    stock_raw_count = len(stock_candidates)
                    candidates, before_relevance = self._prepare_candidates(
                        stock_candidates, request, search_plan, warnings, excluded_sources, strict_relevance=False
                    )
                    logger.info(
                    ...
                    if before_relevance != len(candidates):
                        logger.info(
                            "research.stock_fallback_relevance id=%s kept=%s rejected=%s",
                            request_id,
                            len(candidates),
                            before_relevance - len(candidates),
                        )
            ...
            if not downloaded:
                if (
                    ...
                    stock_candidates = await self._search_stock(search_plan, request, warnings)
                    stock_raw_count = len(stock_candidates)
                    stock_candidates, stock_before_relevance = self._prepare_candidates(
                        stock_candidates, request, search_plan, warnings, excluded_sources, strict_relevance=False
                    )
                    logger.info(
                        "research.stock_fallback id=%s raw=%s licensed=%s",
                    ...
                    if stock_before_relevance != len(stock_candidates):
                        logger.info(
                            ...
                            request_id,
                            len(stock_candidates),
                            stock_before_relevance - len(stock_candidates),
                        )
    ...
    async def _search_entity(
        ...
        if commons_candidates:
            ...
            logger.info(
                "research.search entity=%s source=wikimedia_commons candidates=%s",
                source_selection.entity_type.value,
                len(commons_candidates),
            )
    ...
    def _metadata_score(
        ...
        if any(term in text for term in {"reenactment", "replica", "memorial", "statue", "costume"}):
            if plan and any(
                term in " ".join(plan.visual_requirements).lower()
                for term in {"authentic", "documentary", "historical"}
            ):
                score -= 0.22
