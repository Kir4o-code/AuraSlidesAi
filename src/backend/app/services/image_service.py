...
from app.schemas.presentation import ImageClass, ImageSource, Presentation, ResolvedImageAsset, Slide, SlideType
...
def _domain_signal_counts(title: str, bullets: list[str], prompt: str) -> dict[str, int]:
    ...
    return {domain: sum(1 for term in terms if term in text) for domain, terms in domain_rules.items()}
...
def _rna_research_prompt(lower_subject: str, lower_slide: str) -> str | None:
    ...
    if "rna" not in lower_subject:
        return None
    query_rules = (
        (("vaccine", "vaccines", "medicine", "therapy", "therapies", "therapeutic"), "mRNA vaccine"),
        (("gene editing", "crispr"), "guide RNA CRISPR"),
        (("rrna", "ribosomal", "ribosome", "protein assembly", "protein production"), "ribosome structure"),
        (("trna", "transfer rna", "protein builder"), "transfer RNA ribosome"),
...
def _research_prompt(slide: Slide, presentation_title: str) -> str:
    ...
    if entity:
        ...
        qualifier = _english_keyword_phrase(
            title, *bullets[:2], prompt, limit_words=2, max_length=20, exclude=entity_tokens
        )
        query = (
            f"{english_entity} {qualifier}".strip()
            if qualifier and qualifier.lower() not in english_entity.lower()
            else english_entity
        )
...
async def enrich_presentation_images(
    ...
    logger.info(
        "Image enrichment complete. resolved_images=%s", sum(1 for slide in image_slides if slide.resolved_image)
    )
    return presentation
