...
def _bullet_list_element(
    *,
    ...
    def list_height(size: int) -> int:
        return sum(max(_text_height(bullet, text_width, size)[0] + row_padding, row_min_height) + row_gap for bullet in bullets)
...
def _layout_title_left_feature_slide(slide, spacing_scale: float, typography_scale: float) -> list[LayoutElement]:
    ...
    if slide.notes:
        ...
        elements.append(_text_element(element_id=f"{slide.id}_notes", region="notes", x=MARGIN_X, y=max(y, 560), width=notes_width, text=slide.notes, font_size=notes_font, align=Alignment.START, min_height=notes_height, note="notes"))

    return elements


def _layout_bullets_slide(slide, spacing_scale: float, typography_scale: float) -> list[LayoutElement]:
    width = _available_width()
...
def _layout_bullets_dense_slide(slide, spacing_scale: float, typography_scale: float) -> list[LayoutElement]:
    width = _available_width()
    ...
    if slide.title:
        ...
        elements.append(_text_element(element_id=f"{slide.id}_title", region="title", x=MARGIN_X, y=y, width=width, text=slide.title, font_size=title_font, align=Alignment.START, min_height=title_height, note="dense title"))
    ...
    if slide.notes:
        ...
        elements.append(_text_element(element_id=f"{slide.id}_notes", region="notes", x=MARGIN_X, y=max(y, 614), width=width, text=slide.notes, font_size=notes_font, align=Alignment.START, min_height=notes_height, note="dense notes"))
    return elements


def _layout_image_bullets_slide(slide, spacing_scale: float, typography_scale: float) -> list[LayoutElement]:
    elements: list[LayoutElement] = []
    left_x = 88
    left_width = 560
    right_x = 708
...
def _layout_image_focus_split_slide(slide, spacing_scale: float, typography_scale: float) -> list[LayoutElement]:
    elements: list[LayoutElement] = []
    left_x = 88
    left_width = 430
    right_x = 548
    ...
    if slide.notes:
        ...
        left_children.append(_text_element(element_id=f"{slide.id}_notes", region="notes", x=0, y=y, width=inner_width, text=slide.notes, font_size=notes_font, align=Alignment.START, min_height=notes_height, note="notes"))

    left_height = max(500, _content_bottom(left_children) + (PANEL_PADDING * 2))
    elements.append(_panel_element(element_id=f"{slide.id}_left", region="body", x=left_x, y=panel_y, width=left_width, height=left_height, children=left_children, note="image focus left column"))
    ...
    elements.append(_panel_element(element_id=f"{slide.id}_image_panel", region="media", x=right_x, y=112, width=right_width, height=536, children=image_children, note="image-focused panel"))
    return elements


def _layout_hero_image_slide(slide, spacing_scale: float, typography_scale: float) -> list[LayoutElement]:
    elements: list[LayoutElement] = []
    left_x = 88
    left_width = 468
    right_x = 616
...
def _layout_comparison_slide(slide, spacing_scale: float, typography_scale: float) -> list[LayoutElement]:
    elements: list[LayoutElement] = []
    width = _available_width()
    ...
    def panel(side: str, x: int, heading: str | None, bullets: list[str]) -> LayoutElement:
        ...
        if heading:
            ...
            children.append(_text_element(element_id=f"{slide.id}_{side}_heading", region=f"{side}.heading", x=0, y=0, width=panel_width - 40, text=heading, font_size=heading_font, align=Alignment.START, min_height=heading_height, max_height=52, min_font_size=14, note=f"{side} heading"))
...
def _layout_timeline_slide(slide, spacing_scale: float, typography_scale: float) -> list[LayoutElement]:
    elements: list[LayoutElement] = []
    width = _available_width()
    title_font = _scale_type(40, typography_scale, 34)
    title_y = 84
...
def _layout_statistics_slide(slide, spacing_scale: float, typography_scale: float) -> list[LayoutElement]:
    elements: list[LayoutElement] = []
    width = _available_width()
    title_font = _scale_type(40, typography_scale, 34)
    title_y = 84
    ...
    for index, stat in enumerate(cards):
        ...
        child_elements.append(_text_element(element_id=f"{slide.id}_stat_{index + 1}_value", region=f"stat.{index + 1}.value", x=0, y=0, width=card_width - 40, text=stat_value, font_size=value_font, align=Alignment.CENTER, min_height=value_h, max_height=58, min_font_size=24, note="stat value"))
        label_gap = _gap(10, spacing_scale, 8)
        child_elements.append(_text_element(element_id=f"{slide.id}_stat_{index + 1}_label", region=f"stat.{index + 1}.label", x=0, y=value_h + label_gap, width=card_width - 40, text=stat_label, font_size=label_font, align=Alignment.CENTER, min_height=label_h, max_height=42, min_font_size=12, note="stat label"))
        if stat_detail:
            ...
            child_elements.append(_text_element(element_id=f"{slide.id}_stat_{index + 1}_detail", region=f"stat.{index + 1}.detail", x=0, y=detail_y, width=card_width - 40, text=stat_detail, font_size=detail_font, align=Alignment.CENTER, min_height=detail_h, max_height=56, min_font_size=11, note="stat detail"))
...
def _layout_statistics_featured_slide(slide, spacing_scale: float, typography_scale: float) -> list[LayoutElement]:
    elements: list[LayoutElement] = []
    width = _available_width()
    ...
    if slide.title:
        ...
        elements.append(_text_element(element_id=f"{slide.id}_title", region="title", x=MARGIN_X, y=title_y, width=width, text=slide.title, font_size=title_font, align=Alignment.START, min_height=title_height, note="featured statistics title"))
...
def _layout_quote_slide(slide, spacing_scale: float, typography_scale: float) -> list[LayoutElement]:
    elements: list[LayoutElement] = []
    quote_width = 960
...
def _layout_slide(slide, debug_mode: bool, spacing_scale: float, typography_scale: float) -> LayoutedSlide:
    layout_name = slide.layout_name
    if layout_name == "title.centered":
        elements = _layout_title_slide(slide, spacing_scale, typography_scale)
    elif layout_name == "title.left_feature":
        elements = _layout_title_left_feature_slide(slide, spacing_scale, typography_scale)
    elif layout_name == "content.bullets":
        elements = _layout_bullets_slide(slide, spacing_scale, typography_scale)
    elif layout_name == "content.bullets_dense":
        elements = _layout_bullets_dense_slide(slide, spacing_scale, typography_scale)
    elif layout_name == "content.image_split":
        elements = _layout_image_bullets_slide(slide, spacing_scale, typography_scale)
    elif layout_name == "content.image_focus_split":
        elements = _layout_image_focus_split_slide(slide, spacing_scale, typography_scale)
    elif layout_name == "hero.focus":
        elements = _layout_hero_image_slide(slide, spacing_scale, typography_scale)
    elif layout_name == "comparison.split":
        elements = _layout_comparison_slide(slide, spacing_scale, typography_scale)
    elif layout_name == "timeline.stacked":
        elements = _layout_timeline_slide(slide, spacing_scale, typography_scale)
    elif layout_name == "statistics.grid":
        elements = _layout_statistics_slide(slide, spacing_scale, typography_scale)
    elif layout_name == "statistics.featured":
        elements = _layout_statistics_featured_slide(slide, spacing_scale, typography_scale)
    elif layout_name == "quote.centered":
        elements = _layout_quote_slide(slide, spacing_scale, typography_scale)
    else:
        elements = _layout_bullets_slide(slide, spacing_scale, typography_scale)
...
def build_layouted_presentation(document: PresentationDocument, *, debug_mode: bool = False, spacing_scale: float = 1.0, typography_scale: float = 1.0) -> LayoutedPresentationDocument:
