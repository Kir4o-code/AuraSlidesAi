...
class SlideAnalyzer:
    ...
    def _density(
        *,
        ...
        score = bullet_count * 1.5 + paragraph_length / 80 + timeline_items * 2 + comparison_items * 1.5 + stats_count * 2 - image_count * 0.5
        if score >= 10:
            return "high"
        if score >= 4:
            return "medium"
...
class LayoutScorer:
    def __init__(self, theme_styles: set[str]):
    ...
    def score(
        ...
        elif {analysis.density, metadata.density} == {"low", "medium"} or {analysis.density, metadata.density} == {"medium", "high"}:
            score += 0.05
...
class LayoutSelector:
    ...
    def _infer_theme_styles(theme_name: str) -> set[str]:
        ...
        styles.update(tag for tag in tokens.visual_tags if tag in {"technical", "corporate", "playful", "gradient", "minimal", "academic"})
