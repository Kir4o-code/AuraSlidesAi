...
class PdfExporter(BaseExporter):
    ...
    def _convert_pptx_to_pdf(self, libreoffice_path: str, pptx_path: Path, output_path: Path):
        command = [
            libreoffice_path,
            "--headless",
            "--convert-to", "pdf",
    ...
    def _register_fonts(self) -> tuple[str, str]:
        candidates = (
            ...
            ("AuraSans", "AuraSans-Bold", Path(r"C:\Windows\Fonts\segoeui.ttf"), Path(r"C:\Windows\Fonts\segoeuib.ttf")),
            ("AuraSans", "AuraSans-Bold", Path("/usr/share/fonts/truetype/dejavu/DejaVuSans.ttf"), Path("/usr/share/fonts/truetype/dejavu/DejaVuSans-Bold.ttf")),
        )
    ...
    def _draw_text(self, page: canvas.Canvas, element: LayoutElement, x: float, y: float, w: float, h: float, theme: ThemeDefinition) -> None:
        bold = element.kind in {LayoutElementKind.STATISTIC, LayoutElementKind.QUOTE} or "title" in element.region or "heading" in element.region or "value" in element.region
        ...
        page.setFillColor(self._color(theme.tokens.text_secondary if element.region in {"subtitle", "notes", "attribution"} else theme.tokens.text_primary))
    ...
    def _draw_icon(self, page: canvas.Canvas, x: float, y: float, size: float, icon: str, theme: ThemeDefinition) -> None:
        ...
        if icon == "chart":
            for index, height_scale in enumerate((0.28, 0.45, 0.62)):
                page.rect(x + size * (0.27 + index * 0.18), y + size * 0.22, size * 0.1, size * height_scale, fill=0, stroke=1)
        elif icon == "bolt":
    ...
    def _draw_fitted_image(self, page: canvas.Canvas, path: Path, x: float, y: float, w: float, h: float, fit: str) -> None:
    ...
    def _draw_element(self, page: canvas.Canvas, element: LayoutElement, theme: ThemeDefinition, slide_width: int, slide_height: int, ox: int = 0, oy: int = 0) -> None:
        ...
        if element.kind == LayoutElementKind.PANEL:
            ...
            if self._component_style(theme, "panel").get("style") == "square":
                ...
                page.roundRect(x, y, w, h, float(self._component_style(theme, "panel").get("radius") or 8) * 0.36, fill=1, stroke=1)
            ...
            if image_path and image_path.exists():
                ...
                self._draw_fitted_image(page, image_path, x + inset, y + inset, max(1, w - inset * 2), max(1, h - inset * 2), str(element.content.get("fit") or self._component_style(theme, "image").get("fit") or "cover"))
            ...
            self._draw_icon(page, x + 8, y + h - icon_size - 5, icon_size, str(element.content.get("icon") or "target"), theme)
